...
專案遵循 `Fin_WMAI_BDD_V1.0_20260213.md` 規格書，包含六大功能特性：

1. Feature 1：啟蒙與目標設定
2. Feature 2：風險與能力評估
3. Feature 3：AI 白話化投資建議
4. Feature 4：交易執行與合規
5. Feature 5：資產監控儀表板
6. Feature 6：社交分享功能
...
├── 系統維護手冊.md               # 系統管理指南
├── 開發維護手冊.md               # 開發者文件（本文件）
└── Fin_WMAI_BDD_V1.0_20260213.md # BDD 規格書
```
...
```
    ...
    ▼        ▼                    ▼
┌────────┐ ┌──────────────┐ ┌──────────┐
│chatbot │ │data-service  │ │pages/*.js│
│   .js  │ │     .js      │ │          │
└────┬───┘ └──────┬───────┘ └──────────┘
     │            │
     ▼            ▼
┌─────────┐ ┌───────────────┐
│ Ollama  │ │demo-data.json │
│  API    │ │               │
└─────────┘ └───────────────┘
```
...
const OllamaConfig = {
    ...
    enabled: true,
    timeout: 60000,
    systemPrompt: `你是「小雲」，一位親切、專業的智慧理財小助手。
- 使用繁體中文回答
- 說話親切友善，適時使用表情符號
- 專注於投資理財相關話題
- 回答簡潔明瞭，使用列點和分段
