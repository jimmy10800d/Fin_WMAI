...
│   │   └── pages/           # 各頁面模組 (9 個，含 assistant.js)
│   ├── data/                # Demo 資料
...
伺服器內建 Agent Demo 模擬端點，提供意圖分類、工具鏈執行、護欄拒絕、RAG 引用等功能：
...
# 測試意圖分類
Invoke-RestMethod -Uri "http://localhost:3000/api/intent/classify" -Method Post `
  -Body '{"text":"我聽不懂最大回撤"}' -ContentType "application/json"

# 測試 Agent 工具鏈
Invoke-RestMethod -Uri "http://localhost:3000/api/agent/step" -Method Post `
  -Body '{"userId":"demo","sessionId":"test","text":"我聽不懂最大回撤"}' -ContentType "application/json"
```

### 5.3 AI 助理 API

管理對話記憶、排程任務、目標計畫的 REST API：

| 端點 | 方法 | 說明 |
|------|------|------|
| `/api/assistant/memory` | GET/POST/PATCH/DELETE | 對話記憶 CRUD（含 bulk、pin、clear） |
| `/api/assistant/schedules` | GET/POST/PATCH/DELETE | 排程任務 CRUD（含 enable/disable） |
| `/api/assistant/schedules/:id/trigger` | POST | 手動觸發排程（如每週戰績回報） |
| `/api/assistant/plans` | GET/POST/PATCH/DELETE | 目標計畫與里程碑 CRUD |
| `/api/ollama/health` | GET | Ollama 代理健康檢查 |
| `/api/ollama/chat` | POST | Ollama 對話代理（非串流） |
| `/api/ollama/chat/stream` | POST | Ollama 串流對話代理 |

```powershell
# 範例：查詢對話記憶
Invoke-RestMethod -Uri "http://localhost:3000/api/assistant/memory?userId=demo" -Method Get

# 範例：查詢排程任務
Invoke-RestMethod -Uri "http://localhost:3000/api/assistant/schedules?userId=demo" -Method Get

# 範例：手動觸發每週戰績回報
Invoke-RestMethod -Uri "http://localhost:3000/api/assistant/schedules/<id>/trigger" -Method Post `
  -Body '{"userId":"demo"}' -ContentType "application/json"
```
...
# 2. 檢查 Ollama 服務（透過 proxy）
Invoke-RestMethod -Uri "http://localhost:3000/api/ollama/health" -Method Get

# 3. 若 proxy 不通，直接測試 Ollama
ollama list
...
> **小提示**：chatbot 採混合模式——Agent Demo 優先處理意圖/工具鏈/護欄，Ollama 負責自然語言潤色。兩者可獨立運作，任一不可用時自動 fallback。
