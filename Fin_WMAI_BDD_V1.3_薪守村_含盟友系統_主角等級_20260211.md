...
# 薪守村（Fin_WMAI）系統 BDD 規格書（V1.3：含盟友系統＋主角等級，不含金流）
...
Feature: 目標切入與語意轉換（去門檻）
  ...
  Scenario: 用戶輸入模糊或無法解析，系統引導重新輸入
    Given 用戶進入 App/網銀互動介面
    When 用戶輸入模糊或不完整的目標描述（例：「我想變有錢」）
    Then AI 策略引擎應回傳語意轉換失敗提示
    And 互動介面應提供引導範例（例：「試試看輸入：3 年後出國留學，需要 50 萬」）
    And 系統應記錄 semantic_transform_failed 事件（含原始輸入與失敗原因）
    And 用戶可重新輸入目標描述
```
...
Feature: KYC/能力評估與合規審視
  ...
  Scenario: 合規審視完成後才可進入建議
    ...
    And 系統記錄 compliance_reviewed 事件
    And 才能進入「白話建議」流程
```
...
Feature: 策略匹配與去門檻藍圖生成
  ...
  Scenario: 查無匹配策略，系統提供替代方案
    Given 系統已取得 risk_grade 與 capability_profile
    When AI 策略引擎向「用戶標籤與市場資料庫」查詢策略候選
    And 資料庫回傳空結果（無符合條件的策略）
    Then 系統應提示用戶「目前條件暫無完全匹配的策略」
    And 提供替代建議（放寬條件/調整目標期程/調整投入金額）
    And 系統記錄 strategy_match_empty 事件
```
...
Feature: 即時語意修正（聽不懂）
  ...
  Scenario: 用戶二次點擊「聽不懂」，系統提供轉介真人出口
    Given 系統已嘗試切換一次解釋策略
    When 用戶再次點擊「聽不懂」
    Then 系統應嘗試第二種解釋策略
    And 同時顯示「轉介真人顧問」入口
    And 系統記錄 explainability_escalated 事件（term_id, retry_count, timestamp）
```
...
Feature: 專屬客製化方案（人生目標為核心）
  ...
  Scenario: 核准商品池無法匹配用戶需求（空結果）
    Given 系統已完成初步策略匹配與白話說明
    When 用戶調整偏好後 AI 策略引擎生成的 product_pool_filtered 為空
    Then 系統應提示「目前核准商品池中無完全匹配的商品」
    And 提供替代方案（調整投入頻率/放寬流動性需求/延長目標期程）
    And 系統記錄 product_pool_empty 事件
    And 提供「轉介真人」入口
```
...
```gherkin
Feature: 24/7 監控、再平衡與里程碑回饋（戰績回顧暨戰術調整）
  作為用戶
...
Feature: 信任溫度計（回饋閉環）
  ...
  Scenario: 用戶回報高壓力或低透明度，系統即時調整
    Given 用戶已提交信任溫度計回饋
    When 回饋顯示壓力感為「高」或透明度為「不足」
    Then 系統應立即觸發調整策略：
      | 觸發條件 | 即時反應 |
      | 高壓力 | 降低推播頻率、切換為更溫和的解釋語氣 |
      | 低透明度 | 下次建議自動附帶更詳細的引用來源與計算過程 |
    And 系統記錄 feedback_action_triggered 事件（feedback_id, action_type）
    And 若連續 3 次負面回饋，系統應推薦「轉介真人」
```
...
Feature: 盟友邀請與權限（不含金流）
  ...
  Background:
    ...
    And 單一用戶盟友上限為 10 人，單一任務卡可分享盟友上限為 5 人（Prototype 預設值）
  ...
  Scenario: 盟友人數達到上限，系統拒絕新增
    Given 用戶已擁有 10 位盟友（達上限）
    When 用戶嘗試邀請新盟友
    Then 系統應拒絕產生邀請連結
    And 提示「盟友人數已達上限，請先移除現有盟友後再邀請」
...
```gherkin
Feature: 共同挑戰（Challenge）與 streak
  作為用戶
  我希望與盟友一起完成挑戰
...
# 6B. 主角等級（Leveling System：6 段位 × 5 星，不含金流）

> 目標：用「理解度 × 紀律度 × 自控度」刻畫用戶成長，提升持續率與信任；**不以金額/報酬決定等級**。  
> 呈現：**段位（Rank）= 1–6**，每段 **星等（Stars）= 1–5**（共 30 個成長點）。  
> 解鎖：只解鎖「更好的陪跑工具（解釋/提醒/儀表板/盟友玩法）」；不解鎖更高風險商品能力。

## 6B.1 段位定義（Ranks）
- **R1 啟程者**：完成第 1 張目標任務卡（Quest）＋基本風險理解確認
- **R2 受訓者**：完成 1 次白話建議閱讀＋理解確認，並完成第 1 個週任務
- **R3 紀律者**：達成 streak（例：連續 4 週完成週任務）
- **R4 自控者**：波動事件下按流程完成「先揭露→再決策」並留痕
- **R5 戰術者**：完成 1 次再平衡決策（採納或不採納皆可，但需提交原因）
- **R6 夥伴型玩家（可選）**：長期穩定 streak（例：12 週）＋低壓力回饋穩定，且無違規行為

## 6B.2 星等計算（Stars）
- 每段位內以「XP（任務點數）」推進星等：★1→★5  
- XP 來源僅限「行為任務」與「理解檢核」：
  - 週任務完成（quest_weekly_completed）
  - 風險揭露確認（risk_disclosure_acknowledged）
  - 「聽不懂」改寫後理解回饋（re_explain_feedback_submitted）
  - 信任溫度計回饋（trust_thermometer_submitted）
  - 盟友互動（encourage_received / challenge_completed）— 僅加少量 XP

> 防刷：同類事件每日/每週上限；同一任務重複提交不計分；違規訊息會扣分或凍結進度（Prototype 可先做凍結）。  
> **Prototype 建議上限值**：信任溫度計每日上限 2 次；週任務 XP 每週上限 1 次；風險揭露確認每日上限 3 次；「聽不懂」改寫回饋每日上限 3 次；盟友互動 XP 每日上限 2 次。

---

## Feature O：主角等級計算與升階（Rank & Stars）

```gherkin
Feature: 主角等級計算與升階（不含金流）
  作為用戶
  我希望系統能根據我完成的任務與理解檢核來提升等級
  讓我感受到成長，並解鎖更好的陪跑工具

  Background:
    Given 用戶已登入 Fin_WMAI
    And 系統已建立用戶的 leveling_profile（rank, stars, xp, streak）

  Scenario: 完成任務後獲得 XP 並推進星等
    Given 用戶當前為 Rank 2 Stars 3
    When 用戶完成週任務（quest_weekly_completed）
    Then 系統應加計 XP（依規則與每日/每週上限）
    And 若 XP 達到升星門檻，stars 應提升（★3→★4）
    And 系統記錄 xp_awarded 與 star_up 事件

  Scenario: 升階（Rank Up）必須完成關鍵門檻任務
    Given 用戶當前為 Rank 2 Stars 5
    When 用戶達成關鍵門檻任務（例：streak >= 4 週）
    Then 系統應將用戶 rank 提升至 Rank 3
    And 系統應發放升階徽章與解鎖內容
    And 系統記錄 level_up 事件（from_rank, to_rank, reason_codes）

  Scenario: 波動事件下的「先揭露→再決策」會影響升階
    Given 系統偵測波動並觸發再平衡建議
    When 用戶先完成風險揭露確認
    And 用戶選擇採納或不採納再平衡，並提交原因
    Then 系統應記錄 composure_check_passed
    And 此行為可作為 Rank 4/5 的升階條件之一

  Scenario: 防刷機制—同類事件超過上限不再計分
    Given 用戶今日已達到「信任溫度計」計分上限
    When 用戶再次提交信任溫度計回饋
    Then 系統仍應保存回饋資料
    But 不應再加計 XP
    And 系統記錄 xp_capped 事件
```

---

## Feature P：等級解鎖（Unlocks）
...
Feature: 等級解鎖（不含金流）
  ...
  Scenario: 解鎖「聽不懂」改寫與更完整的解釋模式
    Given 用戶從 Rank 1 升級到 Rank 2
    Then 系統應解鎖「聽不懂」按鈕與更多比喻庫
    And 記錄 unlock_granted（feature=re_explain_modes）

  Scenario: 解鎖「共同挑戰」與 streak 視覺化
    Given 用戶從 Rank 2 升級到 Rank 3
    Then 系統應解鎖共同挑戰（challenge）與 streak 儀表板元件
    And 記錄 unlock_granted（feature=challenges）

  Scenario: 解鎖再平衡工具（仍需合規/風控門檻）
    Given 用戶從 Rank 3 升級到 Rank 4
    Then 系統可解鎖再平衡建議的更完整視覺化
    But 任何調整仍必須通過 Pre-trade Check

  Scenario: 解鎖進階決策分析（再平衡原因回顧）
    Given 用戶從 Rank 4 升級到 Rank 5
    Then 系統應解鎖「再平衡決策回顧」功能（歷次決策對照與影響分析）
    And 記錄 unlock_granted（feature=rebalancing_review）

  Scenario: 解鎖夥伴型玩家專屬工具（可選）
    Given 用戶從 Rank 5 升級到 Rank 6
    Then 系統應解鎖「長期趨勢摘要」與「年度回顧報告」視覺化
    And 系統應解鎖盟友進階功能（發起大型挑戰、設定盟友小隊）
    And 記錄 unlock_granted（feature=partner_tools）
    But 任何交易行為仍必須通過 Pre-trade Check
```
...
- 主角等級不得以金額/報酬作為升級依據；升級依據需可稽核（事件、門檻、原因碼）。
- 需具備防刷設計：同類事件計分上限、重複提交不計分、違規互動可凍結進度。
...
- `level_up`
- `xp_capped`
- `composure_check_passed`
- `unlock_granted`
- `semantic_transform_failed`
- `strategy_match_empty`
- `product_pool_empty`
- `explainability_escalated`
- `feedback_action_triggered`
...
- `POST /profiles/compliance-review`（compliance_reviewed）
...
- `GET  /quests/{quest_id}/allies/visibility-preview`（分享預覽：確認遮蔽效果）
...
## 9.9 Leveling（主角等級）
- `GET  /progress/level`（查詢 rank/stars/xp/streak 與解鎖內容）
- `GET  /progress/unlocks`（查詢已解鎖功能清單）
- `POST /progress/xp`（提交行為事件，計算 XP：含上限與原因碼）
- `POST /progress/evaluate`（每日/每週等級評估：升星/升階/解鎖）
...
- 主角等級系統（段位/星等/XP/解鎖）：6 段位 × 5 星等，行為驅動升級
